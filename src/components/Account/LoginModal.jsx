...
const LoginModal = () => {
	...
	return (
		<>
			{Login && (
				<div className=" modal absolute top-[0vh] left-[0vw] overflow-y-hidden wrapper w-screen h-screen transition-all">
					<div className="wrapper w-full h-full flex justify-center items-center bg-gradient-to-r from-green-700/60 via-white/20 to-red-700/60">
						<motion.div
							className="modal w-[80%] md:w-[45%] xl:w-[25%] h-[60%] md:h-[60%] bg-white rounded-2xl shadow-xl justify-center shadow-gray-600/70 flex flex-col p-2"
							variants={Show}
							initial="initial"
							animate="animate"
						>
							{!loading ? (
								<>
									<div
										className="close-icon text-bold self-end cursor-pointer hover:text-red-500 top-0 relative mr-2 top-[-5%]"
										onClick={handleClose}
									>
										<CloseIcon />
									</div>
